...
THRESHOLD = 5
OUTPUT_PATH = f"ablation_citation_gt_{THRESHOLD}_results_xgboost.csv"
...
print(f"\nCitation prediction target: citation_count > {THRESHOLD}")
...
publication_source_features = startswith_existing(["publication_source"], train_df)
...
feature_groups = {
    ...
    "publication_metadata": publication_metadata_features + publication_source_features,
    "reference_count": reference_count,
...
def combine_groups(group_names):
    features = []
...
# ============================================================
# Ablation configs aligned with thesis model numbering
# ============================================================

ablation_configs = {
    "M0_full_model": full_features,

    # Main feature-group removal ablations
    "M1_remove_temporal_features": [
        f for f in full_features
        if f not in set(feature_groups["temporal"])
    ],
    "M2_remove_text_length_structural_features": [
        f for f in full_features
        if f not in set(feature_groups["text_structure"])
    ],
    "M3_remove_publication_indexing_metadata": [
        f for f in full_features
        if f not in set(feature_groups["publication_metadata"])
    ],
    "M4_remove_reference_count": [
        f for f in full_features
        if f not in set(feature_groups["reference_count"])
    ],
    "M5_remove_research_field_features": [
        f for f in full_features
        if f not in set(feature_groups["research_field"])
    ],
    "M6_remove_author_impact_features": [
        f for f in full_features
        if f not in set(feature_groups["author_impact"])
    ],
    "M7_remove_linguistic_quality_features": [
        f for f in full_features
        if f not in set(feature_groups["linguistic_quality"])
    ],

    # Standalone main feature-group models
    "M8_temporal_features_only": feature_groups["temporal"],
    "M9_text_length_structural_features_only": feature_groups["text_structure"],
    "M10_publication_indexing_metadata_only": feature_groups["publication_metadata"],
    ...
    # Linguistic subgroup removal ablations
    "M15_remove_paper_readability": [
        f for f in full_features
        if f not in set(feature_groups["paper_readability"])
    ],
    "M16_remove_abstract_readability": [
        f for f in full_features
        if f not in set(feature_groups["abstract_readability"])
    ],
    "M17_remove_perplexity": [
        f for f in full_features
        if f not in set(feature_groups["perplexity"])
    ],
    "M18_remove_grammatical_accuracy": [
        f for f in full_features
        if f not in set(feature_groups["grammar"])
    ],
    "M19_remove_native_like_language_use": [
        f for f in full_features
        if f not in set(feature_groups["nativeness"])
    ],

    # Standalone linguistic subgroup models
...
# Keep thesis numbering order instead of sorting by performance
